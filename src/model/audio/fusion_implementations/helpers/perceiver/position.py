...
def positions(
    b, n, shift: Optional[torch.Tensor] = None, device: Optional[torch.device] = None
):
    ...
    if shift is not None:
        if shift.shape != (b, 1):
            raise ValueError(
                f"shift must have shape {(1, b)} but has shape {shift.shape}"
            )
        pos = pos - shift
...
class FourierPositionEncoding(nn.Module):
    ...
    def _position_encodings(
        ...
        frequencies = [
            torch.linspace(
                1.0, max_freq / 2.0, self.num_frequency_bands, device=p.device
            )
