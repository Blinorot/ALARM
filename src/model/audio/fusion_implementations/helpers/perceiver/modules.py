...
from src.model.audio.fusion_implementations.helpers.perceiver.position import (
    RotaryPositionEmbedding,
    positions,
)
from src.model.audio.fusion_implementations.helpers.perceiver.utils import (
    ModuleOutput,
    Residual,
    init_parameters,
)
...
class AbstractAttentionLayer(nn.Sequential):
    def empty_kv_cache(self, x) -> KVCache:
        k_cache = torch.empty(
            x.shape[0], 0, self.num_qk_channels, dtype=x.dtype, device=x.device
        )
        v_cache = torch.empty(
            x.shape[0], 0, self.num_v_channels, dtype=x.dtype, device=x.device
        )
    ...
    def forward(self, *args, kv_cache: Optional[KVCache] = None, **kwargs):
        ...
        return ModuleOutput(
            last_hidden_state=mlp_output.last_hidden_state,
...
class CrossAttentionLayer(AbstractAttentionLayer):
    def __init__(
        ...
        super().__init__(
            Residual(cross_attn, residual_dropout)
...
class SelfAttentionLayer(AbstractAttentionLayer):
    def __init__(
        ...
        super().__init__(
            ...
            Residual(
                MLP(num_channels, widening_factor, bias=mlp_bias), residual_dropout
            ),
        )
...
class SelfAttentionBlock(nn.Sequential):
    def __init__(
        ...
        if activation_checkpointing:
            layers = [
                activation_checkpoint_wrapper(
                    layer, offload_to_cpu=activation_offloading
                )
                for layer in layers
            ]
    ...
    def forward(
        ...
        for i, layer in enumerate(self):
            ...
            output = layer(
                x, pad_mask=pad_mask, rot_pos_emb=rot_pos_emb_i, kv_cache=kv_cache_i
            )
...
class PerceiverEncoder(nn.Module):
    def __init__(
        ...
        self.latent_provider = TrainableQueryProvider(
            num_latents, num_latent_channels, init_scale=init_scale
        )
    ...
    def extra_cross_attention_layer(self):
        return (
            self.num_cross_attention_layers > 1
            and not self.first_cross_attention_layer_shared
        )

    @property
    def extra_self_attention_block(self):
        return (
            self.num_self_attention_blocks > 1
            and not self.first_self_attention_block_shared
        )
    ...
    def forward(self, x, pad_mask=None, return_adapted_input=False):
        ...
        x_latent = self.cross_attn_1(
            x_latent, x_adapted, pad_mask=pad_mask
        ).last_hidden_state
        x_latent = self.self_attn_1(x_latent).last_hidden_state

        cross_attn_n = (
            self.cross_attn_n if self.extra_cross_attention_layer else self.cross_attn_1
        )
        self_attn_n = (
            self.self_attn_n if self.extra_self_attention_block else self.self_attn_1
        )
...
class PerceiverDecoder(nn.Module):
    def __init__(
        ...
        if activation_checkpointing:
            cross_attn = activation_checkpoint_wrapper(
...
class PerceiverAR(nn.Module):
    ...
    def forward(
        ...
        x, frq_pos_enc = self.input_adapter(
            x, abs_pos=positions(b, n, shift=shift, device=x.device)
        )
        ...
        if self.training and prefix_len > 0 and self.cross_attention_dropout > 0.0:
            ...
            keep_mask = torch.zeros_like(rand, dtype=torch.bool).scatter_(
                dim=1, index=keep_indices, value=1
            )
            ...
            frq_pos_enc_prefix = rearrange(
                frq_pos_enc_prefix[keep_mask], "(b n) f -> b n f", b=b
            )

            if pad_mask is not None:
                # drop positions in prefix pad mask
                pad_mask_prefix = rearrange(
                    pad_mask_prefix[keep_mask], "(b n) -> b n", b=b
                )
        ...
        return ModuleOutput(
            last_hidden_state=sa_output.last_hidden_state, kv_cache=kv_cache_updated
        )
...
def activation_checkpoint_wrapper(
    module: AbstractAttentionLayer, offload_to_cpu: bool = False
):
    ...
    def _abstract_attention_layer_patched_forward(self, *args, **kwargs):
        ...
        if (
            hasattr(self, "_activation_checkpointing_enabled")
            and self.training
            and isinstance(output, ModuleOutput)
        ):
